...
public abstract class BaseOutputFormat<K extends WritableComparable<?>, V extends Writable>
	...
	public class MultiRecordWriter extends RecordWriter<K, V> {
		...
		private RecordWriter<K, V> getBaseRecordWriter(TaskAttemptContext job, String baseName)
			...
			//decide the separator between key and value
			...
			if (isCompressed) {
				...
				recordWriter = new MyRecordWriter<K, V>(new DataOutputStream(codec
						.createOutputStream(fileOut)), keyValueSeparator);
			} else {
				Path file = new Path(workPath, baseName);
				FSDataOutputStream fileOut = file.getFileSystem(conf).create(file, false);
				recordWriter = new MyRecordWriter<K, V>(fileOut, keyValueSeparator);
			}
			return recordWriter;
		}
	}
